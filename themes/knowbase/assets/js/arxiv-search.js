...
(function () {
  ...
  function doSearch() {
    ...
    var url = API_BASE +
      '?filter=title.search:' + encodeURIComponent(query) +
    ...
    var cvfPromise = searchCVF(query);
    var dblpPromise = searchDBLP(query);

    Promise.all([oaPromise, s2Promise, cvfPromise, dblpPromise])
    .then(function (responses) {
      loadingDiv.style.display = 'none';
      var oaData = responses[0];
      var s2Result = responses[1];
      var cvfResult = responses[2];
      var dblpResult = responses[3];
      ...
      // Append CVF + DBLP results (deduplicate by title)
      var seenTitles = {};
      merged.forEach(function (w) { seenTitles[(w.title || '').toLowerCase()] = true; });

      // CVF first (has PDF URLs), then DBLP
      [cvfResult.papers, dblpResult.papers].forEach(function (papers) {
        papers.forEach(function (p) {
          var tLower = (p.title || '').toLowerCase();
          if (!seenTitles[tLower]) {
  ...
  var S2_FIELDS = 'title,authors,abstract,externalIds,publicationDate,url,citationCount,openAccessPdf,venue';
    ...
    return fetch(s2Url)
      ...
      .then(function (json) {
        ...
        results.forEach(function (paper) {
          ...
          var doi = ids.DOI || '';
          var oaPdf = paper.openAccessPdf && paper.openAccessPdf.url ? paper.openAccessPdf.url : '';

          // Need arXiv ID, open-access PDF, or recognized venue
          var venueLabel = identifySource(paper.venue || '');
          if (!arxivId && !oaPdf && venueLabel === 'Paper') return;
          ...
          var sourceLabel = 'Paper';
          var absUrl, pdfUrl, paperId;
          ...
          if (arxivId) {
            ...
            // Non-arXiv paper (conference/open-access)
            sourceLabel = venueLabel;
            if (sourceLabel === 'Paper' && /thecvf\.com/.test(oaPdf)) sourceLabel = 'CVF';
            if (sourceLabel === 'Paper' && /ieee\.org/.test(oaPdf)) sourceLabel = 'IEEE';
            absUrl = doi ? 'https://doi.org/' + doi : paper.url || oaPdf || '';
            pdfUrl = oaPdf;
            paperId = 'doi:' + (doi || paper.paperId);
          }
          ...
          papers.push({
            ...
            authorships: authors,
            primary_location: {
              landing_page_url: absUrl,
              pdf_url: pdfUrl,
              source: sourceLabel !== 'arXiv' && sourceLabel !== 'Paper' ? { display_name: paper.venue || sourceLabel } : null
            },
            locations: [],
            _isArxivFallback: true,
            doi: doi,
            _s2Source: sourceLabel
          });
        });
  ...
  // ── DBLP search (via Worker proxy) + CVF static index fallback ──
  function searchDBLP(query) {
    return fetch(WORKER_URL, {
      method: 'POST',
      headers: { 'Content-Type': 'application/json' },
      body: JSON.stringify({ action: 'search-dblp', query: query }),
    })
    .then(function (r) { return r.ok ? r.json() : { results: [] }; })
    .then(function (json) {
      var papers = [];
      (json.results || []).forEach(function (r) {
        if (!r.title) return;
        var venueLabel = identifySource(r.venue);
        // Only include recognized conference/journal papers
        if (venueLabel === 'Paper') return;

        var doi = r.doi || '';
        var authors = (r.authors || []).map(function (a) {
          return { author: { display_name: a } };
        });

        // Construct CVF PDF URL if venue is CVF conference
        var pdfUrl = '';
        var absUrl = r.url || (doi ? 'https://doi.org/' + doi : '');
        if (/CVPR|ICCV|WACV|ECCV|ACCV/.test(venueLabel) && doi) {
          absUrl = 'https://doi.org/' + doi;
        }

        papers.push({
          id: 'dblp:' + (doi || r.title.substring(0, 40)),
          title: r.title,
          publication_date: r.year ? r.year + '-01-01' : '',
          cited_by_count: 0,
          abstract_inverted_index: null,
          _arxiv_abstract: '',
          authorships: authors,
          primary_location: {
            landing_page_url: absUrl,
            pdf_url: pdfUrl,
            source: { display_name: venueLabel !== 'Paper' ? r.venue : '' },
          },
          locations: [],
          _isArxivFallback: true,
          doi: doi,
          _s2Source: venueLabel,
        });
      });
  ...
  // CVF static index as reliable fallback
  var cvfIndex = null;

  function searchCVF(query) {
    var qWords = query.toLowerCase().split(/\s+/).filter(function (w) { return w.length > 1; });
    if (qWords.length === 0) return Promise.resolve({ papers: [] });

    var loadIndex = cvfIndex
      ? Promise.resolve(cvfIndex)
      : fetch('/data/cvf-index.json')
          .then(function (r) { return r.ok ? r.json() : {}; })
          .then(function (data) { cvfIndex = data; return data; })
          .catch(function () { return {}; });

    return loadIndex.then(function (index) {
      var papers = [];
      var confs = Object.keys(index);
      for (var c = 0; c < confs.length; c++) {
        var conf = confs[c];
        var confMatch = conf.match(/^([A-Z]+)(\d{4})$/);
        var confName = confMatch ? confMatch[1] : conf;
        var confYear = confMatch ? confMatch[2] : '';
        var list = index[conf] || [];
        for (var i = 0; i < list.length; i++) {
          var entry = list[i];
          var tLower = entry.t.toLowerCase();
          var allMatch = true;
          for (var j = 0; j < qWords.length; j++) {
            if (tLower.indexOf(qWords[j]) === -1) { allMatch = false; break; }
          }
          if (!allMatch) continue;

          var pdfUrl = 'https://openaccess.thecvf.com' + entry.p;
          var absUrl = 'https://openaccess.thecvf.com' + entry.p.replace('/papers/', '/html/').replace('_paper.pdf', '_paper.html');

          papers.push({
            id: 'cvf:' + conf + ':' + entry.t.substring(0, 40),
            title: entry.t,
            publication_date: confYear ? confYear + '-01-01' : '',
            cited_by_count: 0,
            abstract_inverted_index: null,
            _arxiv_abstract: '',
            authorships: [],
      ...
      return { papers: papers.slice(0, 20) };
    });
  }
  ...
  function extractPaperInfo(work) {
    ...
    if (loc) {
      ...
      if (sourceLabel !== 'Paper') {
        ...
        // Find PDF URL — check primary_location first, then best_oa_location
        var confPdf = pdfField || '';
        if (!confPdf) {
    ...
    // 5) Fallback: any open-access paper with a PDF URL (CVF, IEEE, etc.)
    var oaPdf = '';
    var oaPage = '';
    var boa2 = work.best_oa_location;
    if (boa2 && boa2.pdf_url) { oaPdf = boa2.pdf_url; oaPage = boa2.landing_page_url || ''; }
    if (!oaPdf && loc && loc.pdf_url) { oaPdf = loc.pdf_url; oaPage = loc.landing_page_url || ''; }
    if (!oaPdf) {
      for (var m = 0; m < locations.length; m++) {
        if (locations[m].pdf_url) { oaPdf = locations[m].pdf_url; oaPage = locations[m].landing_page_url || ''; break; }
      }
    }
    if (oaPdf) {
      var oaDoi = (work.doi || '').replace(/^https?:\/\/doi\.org\//, '');
      var oaVenue = loc && loc.source && loc.source.display_name ? loc.source.display_name : '';
      var oaLabel = identifySource(oaVenue);
      if (oaLabel === 'Paper' && /thecvf\.com/.test(oaPdf)) oaLabel = 'CVF';
      if (oaLabel === 'Paper' && /ieee\.org/.test(oaPdf)) oaLabel = 'IEEE';
      return {
        source: oaLabel,
        id: oaDoi || 'open-access',
        absUrl: oaPage || (oaDoi ? 'https://doi.org/' + oaDoi : oaPdf),
        pdfUrl: oaPdf,
        doi: oaDoi,
        arxivUrl: '',
        isConference: oaLabel !== 'Paper'
      };
    }
  ...
  function renderPage() {
    ...
    pagePapers.forEach(function (work) {
      ...
      var badgeClass = info.source === 'bioRxiv' ? ' badge-biorxiv' : ((info.isConference || info.source === 'CVF' || info.source === 'IEEE') ? ' badge-conf' : '');
  ...
  function addPaper(paperUrl, title, btn, topic, doi, pdfUrl, arxivUrl) {
    ...
    // Send doi/pdf_url for conference papers WITHOUT arXiv
    if (!arxivUrl) {
