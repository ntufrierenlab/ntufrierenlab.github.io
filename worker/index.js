...
export default {
  async fetch(request, env) {
    ...
    if (body.action === 'search-dblp') {
      return handleSearchDBLP(body);
    }
...
// ── DBLP search proxy (no CORS on dblp.org) ──────────────────────
async function handleSearchDBLP(body) {
  const query = (body.query || '').trim();
  if (!query || query.length < 2) {
    return jsonResponse(400, { error: 'Query too short' });
  }

  const url = 'https://dblp.org/search/publ/api?q=' +
    encodeURIComponent(query) + '&format=json&h=20';
  ...
  try {
    ...
    if (!res.ok) {
      return jsonResponse(502, { error: 'DBLP returned ' + res.status });
    }
    ...
    const results = hits.map(function (h) {
      const info = h.info || {};
      const ee = info.ee || '';
      // ee can be a string or an object with text
      const eeUrl = typeof ee === 'string' ? ee : (ee.text || '');
      return {
        title: (info.title || '').replace(/<[^>]+>/g, ''),
        venue: info.venue || '',
        year: info.year || '',
        doi: eeUrl.indexOf('doi.org') !== -1 ? eeUrl.replace(/^https?:\/\/doi\.org\//, '') : '',
        url: eeUrl,
        authors: info.authors ? (info.authors.author || []).map(function (a) {
    ...
    return jsonResponse(502, { error: 'DBLP request failed' });
  }
}
