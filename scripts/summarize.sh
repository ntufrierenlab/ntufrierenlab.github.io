...
elif echo "$PAPER_URL" | grep -qi 'thecvf\.com\|ieee\.org' || { [ -n "$EXT_PDF_URL" ] && [ -z "$(echo "$PAPER_URL" | grep -oE '[0-9]{4}\.[0-9]+')" ]; }; then
    # ── Conference paper with HTML page (CVF, IEEE, etc.) ──
    SOURCE="Conference"
    echo "Detected conference paper page: ${PAPER_URL}" >&2

    PAGE_HTML=$(curl -sL "$PAPER_URL")
    if [ -z "$PAGE_HTML" ]; then
        echo "Error: Could not fetch conference paper page" >&2
        exit 1
    fi
...
title = meta('citation_title') or meta('DC.Title') or meta('og:title') or ''
date = meta('citation_publication_date') or meta('DC.Date') or ''
conf_title = meta('citation_conference_title') or ''
abstract = meta('DC.Description') or meta('og:description') or ''
pdf_url = meta('citation_pdf_url') or ''
...
print(f'DATE={date[:10]}')
print(f'ABSTRACT={abstract}')
print(f'CONF={conf_title}')
print(f'PDF_URL={pdf_url}')
" 2>&1)

    # Detect venue from conference title
    CONF_TITLE=$(echo "$METADATA" | grep '^CONF=' | cut -d= -f2-)
    if [ -n "$CONF_TITLE" ]; then
        VENUE_LABEL=$(echo "$CONF_TITLE" | python3 -c "
import sys, re
name = sys.stdin.read().strip().lower()
mappings = [
    (r'cvpr|computer vision and pattern recognition', 'CVPR'),
...
")
    ...
    # Use PDF URL from meta tag or from external argument
    META_PDF=$(echo "$METADATA" | grep '^PDF_URL=' | cut -d= -f2-)
    if [ -n "$EXT_PDF_URL" ]; then
        PDF_URL="$EXT_PDF_URL"
    elif [ -n "$META_PDF" ]; then
        PDF_URL="$META_PDF"
    else
        echo "Error: No PDF URL available" >&2
        exit 1
    fi
    echo "Source: ${SOURCE}" >&2
    echo "PDF URL: ${PDF_URL}" >&2
